        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
            <div class="grid grid-cols-1 md:grid-cols-3">
                <div
                    ...
                    <div
                        class="w-full aspect-square bg-white rounded-xl overflow-hidden border border-gray-200 shadow-sm flex items-center justify-center">
                        @if ($aspiration->photo)
                ...
                <div class="md:col-span-2 p-6 md:p-8">
                    ...
                    <p class="text-gray-600 text-sm mb-8 leading-relaxed whitespace-pre-wrap">{{ $aspiration->description }}
                    </p>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <div class="font-medium text-gray-900 text-sm">
                                {{ $aspiration->category->category_name ?? 'Tanpa Kategori' }}</div>
                        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden p-6 md:p-8">
            ...
            <div class="mb-8">
                <div class="flex items-center flex-wrap gap-3">
                    <span
                        class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium {{ in_array($status, ['pending', 'progress', 'completed', 'rejected']) ? 'bg-blue-600 text-white shadow-sm' : 'bg-gray-100 text-gray-500' }}">
                        Pending
                    </span>
                    ...
                    @if ($status === 'rejected')
                        ...
                        <span
                            class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-red-600 text-white shadow-sm">
                            Rejected
                        </span>
                    @else
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <span
                            class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium {{ in_array($status, ['progress', 'completed']) ? 'bg-yellow-400 text-yellow-900 shadow-sm' : 'bg-gray-100 text-gray-500' }}">
                            Progress
                        </span>
                        ...
                        <span
                            class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium {{ $status === 'completed' ? 'bg-green-600 text-white shadow-sm' : 'bg-gray-100 text-gray-500' }}">
                            Completed
                        </span>
                    @endif
                </div>
                ...
                <div class="flex items-start">
                    ...
                    <div class="w-full">
                        <div class="flex items-center justify-between mb-2">
                            <h6 class="text-sm font-bold text-blue-900 m-0">Feedback Admin</h6>
                            @if ($aspiration->feedback)
                                @if ($aspiration->status === 'progress')
                                    <span class="text-xs font-medium text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full">
                                        {{ $aspiration->feedback->updated_at->format('d M Y, H:i') }}
                                    </span>
